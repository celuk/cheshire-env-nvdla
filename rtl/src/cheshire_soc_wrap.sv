...
`endif
  ...
  `ifdef DRAM_SIM
    ...
    //
    // MIG calibration writes test patterns to row 0, overwriting
    // any preloaded mem_init data. After calibration completes,
    // we reload by calling memory_write directly on each chip.
    //
    // Two x16 chips form 32-bit bus. One BL8 burst = 8 beats × 32 bits
    // = 32 bytes = 2 init lines. Each chip stores 128 bits per entry
    // (8 beats × 16 bits). We read 2 init lines at a time, split:
    //   chip0 gets lower 16 bits of each 32-bit beat
    //   chip1 gets upper 16 bits of each 32-bit beat
    // ---------------------------------------------------------------
    initial begin : sim_mem_init
      integer fd, st, st2, cnt;
      reg [31:0] a0, a1;
      reg [127:0] d0, d1;
      reg [127:0] c0, c1;
      reg [2:0]  bank;
      ...
      // Hold CPU in reset while we wait for calibration and reload data.
      // rst_n = rst_ni & system_reset_o & !uart_dram_mode & pll_locked
      // Forcing uart_dram_mode=1 keeps CPU in reset but keeps dram_wrapper active
      ...
      if (fd == 0) begin
        ...
        cnt = 0;
        while (!$feof(fd)) begin
          // Read pair of init lines (32 bytes = one BL8 burst)
          st = $fscanf(fd, "%h %h", a0, d0);
          if (st > 0) begin
            d1 = 128'h0;
            if (!$feof(fd)) begin
              st2 = $fscanf(fd, "%h %h", a1, d1);
            end

            // Split: chip0 = lower 16 bits of each 32-bit word
            //         chip1 = upper 16 bits of each 32-bit word
            // d0 contains beats 0-3, d1 contains beats 4-7
            c0 = {d1[111:96], d1[79:64], d1[47:32], d1[15:0],
                  d0[111:96], d0[79:64], d0[47:32], d0[15:0]};
            c1 = {d1[127:112], d1[95:80], d1[63:48], d1[31:16],
                  d0[127:112], d0[95:80], d0[63:48], d0[31:16]};

            // Address extraction matching MIG BANK_ROW_COLUMN order:
            //   AXI byte addr[4:0]   → sub-burst offset (32B/burst)
            //   AXI byte addr[11:5]  → col[9:3], col[2:0]=0
            //   AXI byte addr[26:12] → row[14:0]
            //   AXI byte addr[29:27] → bank[2:0]
            // ddr3_model has ROW_BITS=16 but MIG drives only 15 bits;
            // {1'b0, ddr3_addr} pads MSB to 0, so row[15]=0 always.
            bank = a0[29:27];
            row  = {1'b0, a0[26:12]};
            col  = {a0[11:5], 3'b0};

            ddr3_chip0.memory_write(bank, row, col, c0);
            ddr3_chip1.memory_write(bank, row, col, c1);

            if (cnt < 3) begin
              $display("[%0t] SIM_MEM_INIT[%0d]: addr=0x%08h bank=%0h row=%04h col=%04h",
                       $time, cnt, a0, bank, row, col);
              $display("  d0=0x%032h d1=0x%032h", d0, d1);
              $display("  chip0=0x%032h chip1=0x%032h", c0, c1);
            end
            cnt = cnt + 1;
          end
        end
        $fclose(fd);
        $display("[%0t] SIM_MEM_INIT: Loaded %0d burst entries into DDR3.", $time, cnt);
      end

      // Release CPU from reset — now it can safely fetch from DRAM
      release uart_dram_mode;
      $display("[%0t] SIM_MEM_INIT: CPU reset released.", $time);
    end
    `else
    wire ddr3_reset_n;
    wire ddr3_cke;
